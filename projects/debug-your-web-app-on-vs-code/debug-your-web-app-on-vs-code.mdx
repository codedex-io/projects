...
The `addTodo()` function adds a new todo to the state of the app (the `todos` array) when the user creates a new item. It does this by obtaining a variety of values from the `event` object. You will notice that it also calls on `event.preventDefault()`. Since `addTodo()` is being used as a callback function in the `.addEventListener()` function for a form, it needs to prevent the default behaviour of a form. To be specific, `event.preventDefault()` prevents the form from triggering the page to reload, and therefore cause the app to re-render it's state.
