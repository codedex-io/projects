...
Let's add a breakpoint at line 73, beside the `if`-statement that checks if `todo.todoDate` is less than `Date.now()`. Then, let's create a todo with a date from the past. VS Code should pause the app at the breakpoint, and allow you to see the values of the variables until that point.
