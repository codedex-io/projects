...
/>
Now that your debugger is at a good state, you can refer to the project’s [repo](https://github.com/mdwiltfong/codedex_todo/tree/development)
and clone the `development` branch so that you can use the app for this project.

## Beginning the Project

### Project Goals

In order to learn about debugging, buggy code is needed. In this [repo](https://github.com/mdwiltfong/codedex_todo/tree/development) there is a todo app that has a bug built into it. To be specific, the app allows users to enter todo dates in the past, which does not make sense in the context of the app's purpose to help users setup tasks for the future!
...
You can deduce their purposes from their names, but the main functions we will be focusing on is `addTodo()` and `validateTodo()`.

The main purpose of `addTodo()` is to control what todos are being added to the state of the app. It does this by obtaining a variety of values from the `event` object. It also acts as a higher-order function, and accepts `validateTodo()` as a parameter. This is why in the scope of `addTodo()` we have `validateCallback()`, but since `validateTodo()` is the only function being passed into `addTodo()` you can think of `validateCallback()` and `validateTodo()` as the same function.

In order to prevent the user from entering incorret data, the app uses `validateTodo()` to perform a variety of checks on the information the user is submitting. Notice that there is an if-statement that specifically checks if `todo.todoDate` is less than `Date.now()`.
...
Let's add a breakpoint beside the if-statement that checks `todo.todoDate` is less than `Date.now()`. Then, let's create a todo with a date from the past. VS Code should pause the app at the breakpoint, and allow you to see the values of the variables until that point.
...
This isn't right! Let's setup a breakpoint at `todo.todoDate.getTime()` and recreate this scenario in the app to see what's happening.
