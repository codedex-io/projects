...
Have you ever missed an Amazon product that was available at a discounted price? Or dreamt of building an application that notifies you when the price of a product dips below a certain price? If so, then let's save some money together!

In this project tutorial, we will build a Python program that "web scrapes" Amazon product details using a library called [Beautiful Soup](https://www.crummy.com/software/BeautifulSoup/bs4/doc/). 🍜
...
To install the above-mentioned libraries, run the below in your VS Code terminal:
