...
**Prerequisites:** Python fundamentals, Basics of HTML  
...
When we send an HTTP request, we send some **headers** along with the request. If you go to the Network tab of your browser and reload any web page, you can find the request headers similar to below:
...
Now, let us make an HTTP request to get the HTML content of the product page. Using the `BeautifulSoup` class, we will create a soup of the page content.
```python
def get_product_details(product_url: str) -> dict:
...
Now comes the most important part of the tutorial where we will extract data from the page. For the sake of this tutorial, we will extract two data - the **title** of the product and the **price** of the product.

### Extracting the title of the product
Open a product page, say [this one](https://www.amazon.com/Acer-Chromebook-Graphics-802-11ac-CB512-C1KJ/dp/B09Q9YKW15/ref=lp_16225007011_1_2) on your browser, and use the **Inspect Element** option to view the HTML content of the page. You will see the below HTML code for the title of the product.

![Extracting Title](extracting-title.png)
...
```
We use the `find()` method to find a `span` element. We pass the id **productTitle** in a dictionary called `attrs` that accepts the attributes. The `get_text()` method returns the text in a string format. The `strip()` method is used to remove any extra whitespaces.

### Extracting the price of the product

Similar to the title of the product, if you inspect the page, you will find the below HTML code for the price of the product.

![Extracting Price](extracting-price.png)
...
```
We split the `extracted_price` string using the **$** symbol and select the element at index 1. We also add the dollar symbol before the price.
...
As mentioned before, the layout of a website keeps changing and hence, the above code may not be helpful in the future. But this tutorial should be helpful to teach to you how to scrape any website.
