...
Now, let us make an HTTP request to get the HTML content of the product page. Using the `BeautifulSoup` class, we will create a [soup](https://www.crummy.com/software/BeautifulSoup/bs4/doc/#making-the-soup) of the page content.
```python
def get_product_details(product_url: str) -> dict:
...
As mentioned before, the layout of a website keeps changing and hence, the above code may no longer be applicable as it is, without some changes. But this tutorial should be helpful to teach to you how to scrape any website. Join Codédex club to get support from code mentors if you run any trouble while using this article. 
