...
header: https://firebasestorage.googleapis.com/v0/b/codedex-io.appspot.com/o/projects%2Fmake-a-webcam-controlled-game-with-tensorflowjs%2Fbanners.png?alt=media&token=869b25ec-8ac9-45f1-ac14-fd8028ace71c
bannerImage: https://firebasestorage.googleapis.com/v0/b/codedex-io.appspot.com/o/projects%2Fmake-a-webcam-controlled-game-with-tensorflowjs%2Fbanners.png?alt=media&token=869b25ec-8ac9-45f1-ac14-fd8028ace71c
readTime: 60
...
- “Look “Find blob shaped regions”
- “Detect finger-like protrusions”
- “Add max-min width for the size of a finger”
...
- **No server required** - Everything runs on the user’s device.
...
The model can track up to 2 hands simultaneously and runs at about 30 frames per second on most modern devices. That’s super convenient for us!

### How the Detection Process Works

Here’s what happens every time a frame of the live video is processed (You can think of the frame as one image from the video):
...
- The browser shows a permission dialog—users must explicitly grant access.
...
Now we'll create the detection loop that analyzes the webcam feed 30 times per second. This is where the magic happens—every frame, the model finds hands and returns their precise locations!
...
- Model loading takes 1-2 seconds—don't repeat it!
...
While the focus of this project tutorial is not building a game, let’s go over how the ‘machine learning’ part actually connects to the game and how the final code comes together.

I’ve separated all the machine learning code and the camera related logic to be in the `handTracking.js` file, while the game code and rendering the game as well the video on the screen is part of the `game.js` file.
...
- `startGame()` - sets up the ball assets, game state, prepares the canvas. This function is attached to the “Start” button of the game or the “Try again” button of the game.
