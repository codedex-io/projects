...
Here, we are using the `.replace()` method to replace all occurences of the commas `','` with nothing `''`.
...
All we need to do is replace `fig = px.histogram(df, x="subscribers\xa0")` with `fig = px.pie(df, values="subscribers\xa0" names="category\xa", title="YouTube Categories")`:
...
Pie charts are very straightforward to read. The bigger the chunk, the greater quantity we have. We can see here that the top categories in our dataset is Music (24.9%). Coming in at number two is Entertainment (22.1%).
...
The `px.box()` methods follows the convention we have been using in this tutorial:

- First, we called our DataFrame, `df`. 
- Then, we declared out `y` variable, `started\xa0`. 
- Finishing things off, we name our graph “Years Started”. 
