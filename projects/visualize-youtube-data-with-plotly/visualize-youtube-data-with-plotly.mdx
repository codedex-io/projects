...
The `files.upload()` returns a dictionary of the uploaded files. The dictionary's keys are the file names, and the dictionary's values are the file data, based on [Google Colab's documentation](https://colab.research.google.com/notebooks/io.ipynb#scrollTo=BaCkyg5CV5jF) on how to upload files to colab for analysis.
...
This stream of data is finally read by `pd.read_csv()` method and turned into a DataFrame. We store this DataFrame in the `df` variable.
...
Plotly Express allows us to create much more customizable histograms by inputting a variety of arguments. Here's a link to [histogram documentation](https://plotly.com/python/histograms/) for further reference. In this section, we only used the base version with numerical data. But other histograms can also be created based on other columns in the YouTube data.
