...
The next two lines of code sets the `intents` that will be passed into a given instance of `MyClient`. These are the settings for what our Discord can and cannot do. Since we've assigned the `default()` behavior for our bot, we'll need to explicity allow it to interact with messages (i.e., `message_content=True`):
