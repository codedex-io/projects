...
In our `while` loop, we first created a variable called `answer` that will take in an input from the user using the built-in `input()` function.

Using that variable, we then created an `if` statement that will either continue the loop or stop the loop.
