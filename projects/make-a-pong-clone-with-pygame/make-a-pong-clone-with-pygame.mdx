...
**Prerequisites:** Pip, Python fundamentals, knowledge about some game development concepts, such [delta timing](https://en.wikipedia.org/wiki/Delta_timing)
**Versions:** Python 3.10
**Read Time:** X minutes  

## [#](#-introduction) Introduction

Do you know about the first video game ever created? This game is called Pong, and nowadays, 
it's super easy to make. In this tutorial, you'll learn how to make your very own Pong clone, 
...
**NOTE**: This project requires some decent beginner programming experience. I highly recommend you 
do _not_ make this Pong clone as your first project. You will probably not understand much of how 
the code works.

## [#](#-subtitle) Setting Everything Up

Before writing any code, install the PyGame library using Pip:
```sh
pip3 install pygame
```

It's also a good idea to wrap our code in a `main()` function, and to call that function the script is run.
...
Also, add `import pygame` and `import random` at the start of the file to import the PyGame and the random libraries.

## [#](#-subheading) Creating a Window

Before we code any of the game logic, we first need to render a simple window with PyGame. We can
do this very simply:
...
For the ball, we have a `Rect` as well, and two "acceleration" variables (similar to the players' "move" variables). We se those to
a random number between 2 and for to give some speed variety. We also randomize the direction of the ball by multiplying the acceleration
variables by -1, one times out of two. Here is the code for this part (it goes in the **setup** section):
...
Next, let's make the ball move! First we'll need to define two variables in the **setup**: one is the `start` variable,
which determines whether or not the ball can move. This is because we want the ball to only start moving after 3 seconds, to let the players 
move around a bit. The second is a PyGame `Clock` object. This is probably the most important part of the code, since it allows us to tell what 
the time, but most importantly, allow us to determine the [delta time](https://en.wikipedia.org/wiki/Delta_timing). Here is the code for that:
...
In the loop, let's add this little bit of code to tell when the ball can start moving. The number 3000 is an arbitrary number, that is in milliseconds. This
means 3000 is 3 seconds, 4000 would be 4 seconds, etc. You can adjust this to your liking. This code snippet will also include setting the `delta_time` variable.
