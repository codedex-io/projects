...
In our Python program, we start by creating a list that contains the locations of the image files. We also need to create an empty list that will be used to store the actual image data from these files.
...
- `images`: The list of image files
- `duration = 500`: How long each picture should show in the GIF, in milliseconds.
- `loop = 0`: How many times the GIF should repeat (0 means it keeps going forever).
