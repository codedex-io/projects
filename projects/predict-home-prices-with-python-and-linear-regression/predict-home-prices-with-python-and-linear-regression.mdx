...
Next, let's import some data. All data was gathered from [Zillow recently sold](https://bit.ly/3Sawp7f). You will need to download the [data set](https://docs.google.com/spreadsheets/d/11IDKeCPA5jMSG5I_KNps4VfFVfNyG8eIizwnm7KLR4o/edit#gid=0) and upload it to Anaconda.
...
We are using `pd.read_csv()` to read the CSV file containing 59 pairs of data points. Next, we are extracting the values corresponding to the features on the data sheet. In this case, the data from the column named “HouseSize” is being added to the `house_size` variable.
